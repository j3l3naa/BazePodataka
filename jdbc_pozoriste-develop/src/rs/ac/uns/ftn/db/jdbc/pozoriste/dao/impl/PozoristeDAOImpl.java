...
public class PozoristeDAOImpl implements PozoristeDAO {
	...
	public void addTheatresInNewPlaces(Pozoriste p, Mesto m, Drzava d, Scena s) throws SQLException {
		String insertCommand = "insert into pozoriste (nazivpoz, adresapoz, sajt, mesto_idm, idpoz) values (?, ?, ?, ?, ?)";
		...
		try (Connection connection = ConnectionUtil_HikariCP.getConnection();
			...
			mestoDAO.save(connection, m);
			drzavaDAO.save(connection, d);
			
			
			preparedStatement.setString(1, p.getNaziv());
			preparedStatement.setString(2, p.getAdresa());
			preparedStatement.setString(3, p.getSajt());
			preparedStatement.setInt(4, m.getIdm());
			preparedStatement.setInt(5, p.getId());
			preparedStatement.executeUpdate();
			
			scenaDAO.save(connection, s);
